 
  <div class="container-fluid">
    <div class="row" style="align-items: center">
        <div class="col-xl-4 col-sm-4 col-4 border text-center" style="background-color: gray ;height: 50px; " style><a  class="nav-link"  href="{{ route('areaPersonal')}}">Informacion</a></div>
        <div class="col-xl-4 col-sm-4 col-4 border text-center " style="background-color: gray ;height: 50px; "><a class="nav-link" href="{{ route('ventanaPuntuacion')}}">Puntuacion</a></div>
        <div class="col-xl-4 col-sm-4 col-4 border text-center " style="background-color: gray ;height: 50px; "><a class="nav-link" href="{{ route('ventanaPrueba')}}">Grupos</a></div>
    </div>

  </div>
  
  
  
  
  
  
  
  
  {{-- <div class="container-fluid" style="background-color: gray; text-align: center; color:greenyellow" >

    

            <a  class="nav-link"  href="{{ route('ventanaEstadisticas')}}">Estadisticas</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="{{ route('ventanaPuntuacion')}}">Puntuacion</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="{{ route('ventanaGrupos')}}">Grupos</a>
    </div> --}}

  